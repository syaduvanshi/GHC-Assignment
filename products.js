...
let api=  "https://www.snowdevil.ca/14168/checkouts/0123456789abcdef0456456789abcdef/recover?key=6dacd6065bb80268bda857ee"
let data={
    "billing_address": {
    ...
    "closed_at": null,
    "completed_at": null,
    "created_at": "2023-03-10T11:00:00-05:00",
    "currency": {
      "currency": "USD"
    },
    "customer": {
    ...
    "total_tax": "11.94",
    "total_weight": 400,
    "updated_at": "2023-03-07T19:25:35+00:00",
    "user_id": 1
  } 
  let orderData= {
    "id": 450789469,
  ...
  exports.checkifitLiesInFirstMessage = async() =>{
    try{
    let date = new Date (data.updated_at);
    let today = new Date();
    let storingMessage =[];
    let subject="Remainder Mail After 30 Minutes";
    let message="We hope this email finds you well. We noticed that you left some items in your shopping cart on our website and we wanted to remind you that they are still available for purchase"
    const diffTime = Math.abs(date - today);
    var minutes = Math.floor((diffTime/1000)/60);
    //here we are checking if the time difeerence between abandon checkout and current time is 30 min and user has not placed order for that particular checkout order
    if(minutes === 30 && data.cart_token != orderData.cart_token){
        var transporter = nodemailer.createTransport({
          ...
          transporter.sendMail(mailOptions, function(error, info){
            if (error) {
              ...
              console.log('Email sent: ' + info.response,storingMessage);
            }
          });
    }else if(minutes === 30){
      storingMessage.push({
        title:"Order Placed",
        message:"Order Have been Placed after this reminder mail"
      })
      console.log("Records",storingMessage)
    }
    }catch(e){
        console.log(e)
    }
  ...
  exports.checkifitLiesInSecondMessage = async() =>{
    let date = new Date (data.updated_at);
    let today = new Date();
    let storingMessage =[];
    let subject="Remainder Mail After 30 Minutes";
    ...
    if(diffDays === 1 && data.cart_token != orderData.cart_token){
        var transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
              user: process.env.EMAIL,
  ...
  exports.checkifitLiesInThirdMessage = async() =>{
    let date = new Date (data.updated_at);
    let today = new Date();
    let storingMessage =[];
    let subject="Remainder Mail After 30 Minutes";
    ...
    const diffTime = Math.abs(date - today);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    if(diffDays === 3 && data.cart_token != orderData.cart_token){
        var transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
              user: process.env.EMAIL,
